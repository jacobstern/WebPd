...
exports.declareObjects = function(library) {
  ...
  library['throw~'] = PdObject.extend({
      ...
      portlets.DspInlet.extend(_DspArithmValInletMixin, {
        _setValNoDsp: function(val, timeTag) {
          if (typeof val === 'undefined')
            // Happens when connecting a corresponding [catch~] with no input
            return
          if (!_.isNumber(val))
            throw new Error('Invalid input for [throw~] ' + val)
          if (this.obj._offsetNode)
            this.obj._offsetNode.offset.setValueAtTime(val, timeTag / 1000)
        }
      })
    ...
    init: function(args) {
      var name = args[0] || ''
      this._catchReference = new mixins.Reference('catch~')
      this._catchReference.on('changed', this._onCatchReferenceChanged.bind(this))
      this._catchReference.set(name)
    },

    start: function() {
      this._assureGainNode()
    },
    ...
    connectReceiver: function(node) {
      this._offsetNode = new WAAOffset(pdGlob.audio.context)
      this._offsetNode.offset.value = 0
      this._offsetNode.connect(node, 0, 0)

      this._assureGainNode()
      this._gainNode.connect(node)

      if (!this.i(0).hasDspSource() && typeof this.val !== undefined)
        this.i(0)._setValNoDsp(this.val, 0)
    },

    disconnectReceiver: function(node) {
      this._offsetNode = null
    },
    ...
    _onCatchReferenceChanged: function(newObj, oldObj) {
      if (oldObj)
        oldObj.unregisterSender(this)
      if (newObj)
        newObj.registerSender(this)
    },

    _assureGainNode: function() {
      if (this._gainNode)
        return
      this._gainNode = pdGlob.audio.context.createGain()
      this._gainNode.gain.value = 1
      this.i(0).setWaa(this._gainNode)
    },
  ...
  library['catch~'] = PdObject.extend(mixins.NamedMixin, mixins.EventEmitterMixin, {
    type: 'catch~',
    ...
    start: function() {
      ...
      for (var i = 0; i < this._senders.length; i++)
        this._senders[i].connectReceiver(this._gainNode)
      this._started = true
    },
    ...
    stop: function() {
      this._started = false
      for (var i = 0; i < this._senders.length; i++)
        this._senders[i].disconnectReceiver(this._gainNode)
      this._gainNode = null
    },

    registerSender: function(sender) {
      this._senders.push(sender)
      if (this._started)
        sender.connectReceiver(this._gainNode)
    },

    unregisterSender: function(sender) {
      var index = this._senders.indexOf(sender)
      if (index >= 0)
        this._senders.splice(index, 1)
      if (this._started)
        sender.disconnectReceiver(this._gainNode)
    }
  })

  library['s~'] = library['send~'] = PdObject.extend(mixins.NamedMixin, mixins.EventEmitterMixin, {
    type: 'send~',
    ...
    start: function() {
      ...
      for (var i = 0; i < this._receivers.length; i++)
        this._receivers[i].connectSender(this._gainNode)
      this._started = true
    },
    ...
    stop: function() {
      this._started = false
      for (var i = 0; i < this._receivers.length; i++)
        this._receivers[i].disconnectSender(this._gainNode)
      this._gainNode = null
    },
    ...
    unregisterReceiver: function(receiver) {
      var index = this._receivers.indexOf(receiver)
      if (index >= 0)
  ...
  library['r~'] = library['receive~'] = PdObject.extend({
    type: 'receive~',
    ...
    init: function(args) {
      var name = args[0] || ''
      this._sendReference = new mixins.Reference('send~')
      this._sendReference.on('changed', this._onSendReferenceChanged.bind(this))
      this._sendReference.set(name)
    },

    _onSendReferenceChanged: function(newObj, oldObj) {
      if (oldObj)
        oldObj.unregisterReceiver(this)
      if (newObj)
        newObj.registerReceiver(this)
    },
    ...
    connectSender: function(node) {
      this._assureGainNode()
      node.connect(this._gainNode)
    },

    disconnectSender: function(node) {
      if (this._gainNode)
    ...
    _assureGainNode: function() {
      ...
      this._gainNode = pdGlob.audio.context.createGain()
      this._gainNode.gain.value = 1
      this.o(0).setWaa(this._gainNode)
    }
  })
}
