...
exports.declareObjects = function(library) {
  ...
  library['unpack'] = PdObject.extend({
    ...
    inletDefs: [
      ...
      portlets.Inlet.extend({
        message: function(args) {
          var i, filter, arg
          for (i = 0; i < this.obj.filters.length && i < args.length; i++) {
            filter = this.obj.filters[i]
            arg = args[i]
            if ((filter === 'float' || _.isNumber(filter)) && _.isNumber(arg) ||
              filter === 'symbol' && typeof arg === 'string')
              this.obj.o(i).message(utils.timeTag([arg], args))
            else
              console.error('Type mismatch in unpack: ' + filter + ', ' + arg)
          }
        }
      })
    ],
  ...
  _RouteInlet0 = portlets.Inlet.extend({
    message: function(args) {
      this.obj._handleMessage(args)
    }
  })

  _RouteInlet1 = portlets.Inlet.extend({
    message: function(args) {
      if (args.length === 1) {
        this.obj._value = args[0]
      } else {
        console.error('invalid [route] argument value: ' + val)
      }
    }
  })
  ...
  library['route'] = PdObject.extend({
    ...
    init: function(args) {
      ...
      this.inlets = length <= 1
        ? [new _RouteInlet0(this, 0), new _RouteInlet1(this, 1)]
        : [new _RouteInlet0(this, 0)]
      for (var i = 0; i < length + 1; i++) {
        this.outlets.push(new portlets.Outlet(this, i))
      }

      this._type = (length === 0 || _.isNumber(first)) ? 'float' : 'symbol'
      this._args = args
      this._value = length === 0 ? 0 : first
    },

    _handleMessage: function(args) {
      var compare = this._args
        , i
        , sent = false
      if (compare.length <= 1) {
        compare = [this._value]
      ...
      for (i = 0; i < compare.length; i++) {
        if (this._trySendMessage(args, compare[i], i)) {
          sent = true
          break
        }
      }
      if (!sent) {
        this._sendMessage(args, i, false)
      }
    },

    _matchTypeSymbol: function(args, compare) {
      if (args.length === 0 && compare === 'bang')
        return true
      else if (args.length === 1) {
        var first = args[0]
        if (_.isNumber(first) && compare === 'float') 
          return true
        else if (typeof first === 'string' && compare === 'symbol')
          return true
    ...
    _trySendMessage: function(args, compare, outIndex) {
      if (this._matchTypeSymbol(args, compare)) {
        this._sendMessage(args, outIndex, false)
      } else if (args[0] === compare) {
        this._sendMessage(args, outIndex, true)
      } else {
        return false
      }
      return true
    ...
    _sendMessage: function(inputArgs, outIndex, skipFirst) {
      var skip = skipFirst ? 1 : 0
      if (inputArgs.length - skip <= 0) {
        this.o(outIndex).message(utils.timeTag(['bang'], inputArgs))
      } else {
        this.o(outIndex).message(utils.timeTag(inputArgs.slice(skip), inputArgs))
      }
    }
  })
